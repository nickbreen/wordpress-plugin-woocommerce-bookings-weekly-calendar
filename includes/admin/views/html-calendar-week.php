	...
	<form method="get" id="mainform" enctype="multipart/form-data" class="wc_bookings_calendar_form">
		...
		<div class="tablenav">
			...
			<div class="views">
				...
				switch ($view) :
					case self::VCUST :
						?><a class="week-product" href="<?php echo add_query_arg('view', 'week-product'); ?>"><?php _e( 'Week View by Product', 'wordpress-plugin-woocommerce-bookings-weekly-calendar' ); ?></a><?php
						break;
					default:
						?><a class="week-customer" href="<?php echo add_query_arg('view', 'week-customer'); ?>"><?php _e( 'Week View by Customer', 'wordpress-plugin-woocommerce-bookings-weekly-calendar' ); ?></a><?php
				endswitch;
				?>
			...
			<thead>
				<tr>
					...
					<?php for ($ii = get_option('start_of_week', 1); $ii < get_option('start_of_week', 1) + 7; $ii ++) : ?>
						<th width="12.5%"><?php echo date_i18n(_x('l', 'date format', 'wordpress-plugin-woocommerce-bookings-weekly-calendar'), strtotime("next sunday +{$ii} day")); ?></th>
					<?php endfor; ?>
				</tr>
			</thead>
			<tbody>
